...
// Channel recv_channel_p0_from_p4 variable p0_in0
volatile int *p0_in0=(int *)NOC_PARAMETER_MAP(NOC_RNI_RECV_BASE(recv_channel_p0_from_p4),0);
volatile int *p0_in0_heartbeat=(int *)NOC_PARAMETER_MAP(NOC_RNI_RECV_BASE(recv_channel_p0_from_p4),sizeof(int)/4);
...
void p0_main(void)
{
	//int channel=SomethingForMe(p0_pid,p3_pid); // My id, Source id
	//if (channel!=-1)
	//{
	(*p0_out0)=(*p0_in0)+1;
	NOC_RNI_SEND(NOC_RNI_BASE,send_channel_p0_to_p1,p0_out0_priority,p0_out0_msg_len);
